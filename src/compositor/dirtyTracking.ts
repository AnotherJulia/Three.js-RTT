 ...
 * rect plus the intersection with its nearest overflow:hidden/scroll ancestor
...
export function findLiveElements(root: HTMLElement, selector = "video, canvas"): LiveElementRect[] {
  ...
  return elements.map((element) => {
    ...
    let clip: LiveElementRect["clip"] = null;
    ...
    while (ancestor && ancestor !== root.parentElement) {
      ...
      if (style.overflow === "hidden" || style.overflowX === "hidden" || style.overflowY === "hidden") {
        ...
        clip = {
          left: ancestorRect.left - rootRect.left,
          top: ancestorRect.top - rootRect.top,
